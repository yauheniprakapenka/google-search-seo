...
We created the job posting content policies to ensure that our users are connected with jobs that are open, relevant, and easy to apply to. The [general structured data guidelines](/search/docs/appearance/structured-data/sd-policies) and [spam policies for Google web search](/search/docs/essentials/spam-policies) also apply to job postings. If we find content that violates these policies, we'll respond appropriately, which may include taking [manual action](https://support.google.com/webmasters/answer/9044175) and removing the job posting(s) from the job search experience on Google.
...
We don't allow promotional content disguised as a job posting, such as those posted by a third party (for example, [affiliate programs](/search/docs/essentials/spam-policies#thin-affiliate-pages)).
...
For a full list of examples, see the examples in [our content policies](#content-policies). You may have received this Search Console message: "Structured data policy violation - Content on pages found to be different than structured data on the page".
