...
-   Users must be able to submit answers to the question. Don't use `QAPage` markup for content that has only one answer for a given question with no way for users to add alternative answers; instead, use [`FAQPage`](/search/docs/appearance/structured-data/faqpage). Here are some examples:
