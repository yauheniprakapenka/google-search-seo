...
2.  Add the [required properties](#structured-data-type-definitions). Based on the format you're using, learn where to [insert structured data on the page](/search/docs/appearance/structured-data/intro-structured-data#format-placement).
...
List of items. To specify a list, define an `ItemList` that contains at least two `ListItem` elements. All items must be of the same type. See `[ListItem](#list-item)` for details.
...
-   Any other properties required for this data type, as described in schema.org and the rules described in the documents for your content type:
    ...
    **Example**: For a recipe, you would provide `prepTime` and `image` properties.
