...
| `returnPolicyCategory` | `MerchantReturnEnumeration`  The type of return policy. Use one of the following values:  If you use `MerchantReturnFiniteReturnWindow`, then the [`merchantReturnDays`](#merchant-return-days) property is required. |
| Option B | |
...
| `returnFees` | `ReturnFeesEnumeration`  The default type of return fee. Use one of the following supported values: |
| `returnMethod` | `ReturnMethodEnumeration`  The type of return method offered. Use one or more of the following values: |
...
| `itemCondition` | `OfferItemCondition`  The acceptable conditions of an item which can be returned. You can provide multiple conditions which are accepted. Use the following values: |
...
| `refundType` | `RefundType`  The type of refund(s) available for the consumer when returning a product. |
...
| `returnLabelSource` | `ReturnLabelSourceEnumeration`  The method by which the consumer can obtain a return shipping label for a product. Use one of the following values: |
...
| `returnPolicySeasonalOverride.returnPolicyCategory` | `MerchantReturnEnumeration`  The type of return policy. Use one of the following values:  If you use `MerchantReturnFiniteReturnWindow`, then the `merchantReturnDays` property is required. |
