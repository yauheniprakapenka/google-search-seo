...
**Not changing the URLs?** If you are making infrastructure changes such as changing your hosting, [start here instead](https://developers.google.com/search/docs/crawling-indexing/site-move-no-url-changes).

## Overview

1. [**General best practices for site moves**](#general-best-practices-for-site-moves). Know what to expect, and how it might affect your users and rankings. If moving from HTTP to HTTPS, review the [best practices for HTTPS](https://web.dev/articles/enable-https).
2. [**Prepare the new site**](#prepare-the-new-site) and test it thoroughly.
3. [**Prepare a URL mapping**](#prepare-url-mapping) from the current URLs to their corresponding new format.
4. [**Start the site move**](#start-the-site-move) by configuring the server to redirect from the old URLs to the new ones.
5. [**Monitor the traffic**](#monitor-traffic) on both the old and new URLs.
...
1. **Implement or turn on the redirects**: depending on your decisions your redirect strategy, this might mean you push an update to your server configuration files or you update your CMS, likely with custom code.
...
4. **Submit a Change of Address in Search Console for the old site**.

   If you're moving your site from HTTP to HTTPS, you don't need to use the **Change of Address** tool.
