...
- **Region variants**: for example, separate USA and UK URLs with [`hreflang`](/search/docs/specialty/international/localized-versions), but essentially the same content in the same language
...
Different language versions of a single page are considered duplicates only if the primary content is in the same language (that is, if only the header, footer, and other non-critical text is translated, but the body remains the same, then the pages are considered to be duplicates). For regional variants in the same language (for example, a piece of content for the USA and the UK that's accessible at different URLs but in the same language), use both canonicalization and [`hreflang`](/search/docs/specialty/international/localized-versions) to help Google better understand which regional URL to show in search results. To learn more, see our documentation on [handling duplicate pages on multi-regional sites](/search/docs/specialty/international/managing-multi-regional-sites#dup-content).
