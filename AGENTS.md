...
Это офлайн-архив официальной документации Google по SEO (поисковой оптимизации для Google Search), скопированной с `developers.google.com/search/docs`, и дополнительных материалов с мнениями экспертов в папке `experts`. Исполняемого кода нет.
...
- **Всегда заново получать HTML страницы.** Каждый новый аудит должен начинаться с fresh-запроса страницы. Использование ранее закешированного HTML запрещено — содержимое страницы могло измениться с момента прошлого запроса (добавлена разметка, изменён контент, обновлены мета-теги). Если аудит проводится повторно для той же страницы — всё равно делать новый fetch.
...
- После изучения релевантной официальной документации проверить папку `experts` на наличие материалов по теме.
- Сначала изложить требования, выводы и рекомендации из основной документации, затем отдельным блоком привести релевантное мнение экспертов как дополнительную информацию.
- Не подменять мнением эксперта требования Google и не использовать его как единственное основание для нарушения или обязательной рекомендации.
- Для каждого использованного мнения указывать конкретный файл из `experts`.
- Если мнение эксперта противоречит официальной документации, приоритет имеет официальная документация; противоречие следует явно отметить.
...
10. **Стандартный набор схем для страниц ветеринарных клиник.** Для страниц услуг ветеринарной клиники (и аналогичных локальных бизнесов) следует использовать три типа JSON-LD в одном `@graph`. Все схемы на странице объединяются в один `<script type="application/ld+json">` с корневым объектом `{"@context": "https://schema.org", "@graph": [...]}`, а не в несколько отдельных `<script>` с массивами.

    **Формат вывода схем.** При выдаче готового JSON-LD агент всегда оборачивает схему в теги `<script type="application/ld+json"> ... </script>`, чтобы результат был полностью готов для копирования и вставки в HTML. Не отдавать «голый» JSON без тегов.
    ...
    **Используемые типы:**
    ...
    - **`VeterinaryCare`** (не `LocalBusiness`) — наиболее специфичный тип по документации ("Use the most specific LocalBusiness sub-type possible", `05-ranking-and-search-appearance/17-structured-data/features-guides/16-local-business.md`). Свойства: `name`, `description`, `url`, `telephone`, `address` (массив филиалов), `image`, `priceRange`, `openingHoursSpecification`. В `PostalAddress` заполняются только `streetAddress`, `addressLocality`, `addressCountry`; `postalCode` **никогда** не проставляется агентом (см. ограничение #18). Предупреждения валидатора для `priceRange` и `openingHoursSpecification` игнорируются (свойства унаследованы от `LocalBusiness`, Google обрабатывает корректно). `aggregateRating` — **только** при наличии верифицируемых отзывов (см. ограничение #9).
    - **`BreadcrumbList`** — хлебные крошки для понимания структуры сайта. Обязательно для каждой страницы. Последний элемент `ListItem` (текущая страница) не должен содержать свойство `item` — Google определяет текущую страницу по URL, указывать его повторно не нужно.
...
15. **Формат `DateTime` в структурированных данных — ISO 8601 со временем и таймзоной.** Если свойство имеет тип `DateTime` (например, `ProfilePage.dateCreated`, `ProfilePage.dateModified`, `Article.datePublished`, `Event.startDate`), значение должно быть в полном формате ISO 8601: `YYYY-MM-DDTHH:MM:SS±HH:MM` или `YYYY-MM-DDTHH:MM:SSZ`. **Недопустимо** использовать сокращённый формат `YYYY-MM-DD` (это тип `Date`, а не `DateTime`) — валидатор Schema.org и Rich Results Test выдают ошибку «invalid date/time value». Пример правильного значения для Москвы: `"2026-06-05T12:00:00+03:00"`. Источник: `05-ranking-and-search-appearance/17-structured-data/features-guides/21-profile-page.md:147–148` (свойства имеют тип `DateTime`, описание явно требует ISO 8601). Для свойств типа `Date` (например, у `Event` могут быть) — наоборот, использовать `YYYY-MM-DD`. Всегда проверять тип свойства в документации перед проставлением значения.

16. **Ссылка на `logo` в схемах — всегда уточнять у пользователя.** При генерации структурированных данных, содержащих свойство `logo` или `image` для `Organization`/`VeterinaryCare`/`LocalBusiness`/других типов, агент **обязан** явно спросить у пользователя корректный URL логотипа перед выдачей схемы. Ссылка на логотип в HTML страницы (например, в шапке) может быть устаревшей, временной или указывать не на тот ресурс. Если пользователь в рамках текущей сессии уже предоставил URL логотипа — использовать его во всех последующих схемах без повторного вопроса. Если пользователь проигнорировал запрос (не ответил) — считать, что текущая ссылка актуальна, и больше не спрашивать в рамках этой сессии.

17. **`priceRange` — использовать знак `$`, а не `₽`.** Документация (`05-ranking-and-search-appearance/17-structured-data/features-guides/16-local-business.md:495–499`) описывает два формата: числовой диапазон (например, `"$10-15"`) или нормализованное количество знаков валюты (например, `"$$$"`). Во всех примерах в документации используется знак доллара. Использование `₽₽` или других нестандартных символов не поддерживается Google. Всегда использовать `"$$"` для среднего ценового сегмента.

18. **Никогда не выдумывать `postalCode`.** Свойство `postalCode` в `PostalAddress` агент **не заполняет самостоятельно** — ни по памяти, ни по приблизительным данным, ни копируя из существующей разметки сайта без подтверждения пользователем. Почтовые индексы легко перепутать между районами, а неверный индекс напрямую искажает данные локального бизнеса в выдаче.

    **Правило:** во всех генерируемых схемах свойство `postalCode` **опускается** (поле просто не включается в объект `PostalAddress`). Свойства `streetAddress`, `addressLocality`, `addressCountry` заполняются по данным со страницы.

    **Исключение:** если пользователь в рамках сессии явно и напрямую предоставил корректные почтовые индексы (например, перечислил их в сообщении или подтвердил конкретные значения) — использовать их. Перенос индексов из существующей разметки аудируемого сайта **не является подтверждением** и не допускается.

    **Напоминание в конце ответа.** Каждый раз, когда в выданной схеме присутствует `PostalAddress` без `postalCode`, агент **обязан** завершить ответ явным напоминанием формата: «⚠️ Свойство `postalCode` не заполнено намеренно. Впишите актуальные почтовые индексы для каждого адреса самостоятельно перед публикацией разметки.»

19. **После каждой выданной или изменённой схемы — рекомендовать проверку в Rich Results Test.** Каждый раз, когда агент отдаёт пользователю новую или обновлённую JSON-LD-разметку, ответ **обязан** завершаться рекомендацией проверить её через валидаторы Google. Это критично: опечатки в `@type`, невалидные значения `DateTime`, лишние запятые или неполные узлы `@graph` приводят к тому, что Google игнорирует разметку целиком.

    **Формулировка (добавлять в конец ответа со схемой):** «После вставки проверьте разметку через [Rich Results Test](https://search.google.com/test/rich-results) (показывает, какие rich results активируются) и [Schema Markup Validator](https://validator.schema.org/) (полная проверка синтаксиса и структуры `@graph`).»

    **Когда применяется:** при генерации новой схемы, при редактировании существующей, при исправлении ошибок в чужой разметке. **Когда не применяется:** при простом обсуждении схем без выдачи готового кода, при аудите (там проверка через валидатор уже входит в процесс).

    Предупреждения (warnings) валидатора для `priceRange` и `openingHoursSpecification` на типах семейства `LocalBusiness` игнорируются — это известная особенность унаследованных свойств (см. ограничение #10). Реально важны только ошибки (errors).

20. **Страницы экспертов (`experts/`) не проверяются на обновления.** Файлы в папке `experts/` — это сторонние материалы и мнения экспертов, а не официальная документация Google. При проверке актуальности архива (сверке `.md`-файлов с их исходными URL и датами) папку `experts/` **пропускать целиком**: не запрашивать их исходные страницы, не сравнивать даты и не обновлять их содержимое. Страницы экспертов не включаются в отчёты о проверке/синхронизации архива, если пользователь явно не попросил проверить конкретный файл из `experts/`.

21. **Совпадение дат = файл актуален, содержание не сверяется.** Если `> Last updated:` в локальном файле совпадает с датой «Last updated» на веб-странице, содержание файла **не проверяется** — считаем файл актуальным. Сверка содержания выполняется только тогда, когда дата на веб-странице новее локальной.
